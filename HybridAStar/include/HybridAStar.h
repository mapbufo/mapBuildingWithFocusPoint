...
void Update(Node wknode, std::vector<Node> &Open, std::vector<Node> &Close, Vehicle &veh, Configuration &cfg)
{
  ...
  for (auto D : mres_vec)  // D: distance between the current car rear axis and car rear axis at the next prosition
  {
    for (auto delta : sres_vec)  // delta: turning angle resolution
    {
      Node tnode;
...
/**
   * @brief get the final hybrid A* path
   * @param input rspath: calculated Reeds-Shepp path
   * @param input Close: close node list
   * @param input veh: reference of the vehicle data
   * @param input cfg: reference of the configuration file
   * @param output x_vec: x value of the planned path
   * @param output y_vec: y value of the planned path
   * @param output th_vec: theta value of the planned path
   * @param output D_vec: D value of the planned path
   * @param output delta_vec: delta value of the planned path
   */
