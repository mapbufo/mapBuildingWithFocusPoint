...
namespace PathPlanning
{
  ...
  void OptimizePath(std::vector<Point2D> &path)
  {
    for (int i = 0; i < path.size() - 2; i++)
    {
      ...
      {
        ratio_first = (path[i + 1].second - path[i].second) / (path[i + 1].first - path[i].first);
      }
      float ratio_second;
      ...
      {
        ratio_second = (path[i + 2].second - path[i + 1].second) / (path[i + 2].first - path[i + 1].first);
      }
  ...
  std::vector<Point2D> PathPlanning(Point2D start_pos, Point2D end_pos, const Map &map, bool global)
  {
    // result path
