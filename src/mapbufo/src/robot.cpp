...
void Robot::findBestPos(ScanData scan, Point2D candidate_pt, Point2D &best_pos, float &best_heading, bool only_empty)
{
  ...
  if (scan.find({ pt_x, pt_y }) != scan.end())
  {
    ...
    if (interested_status_list.find(scan[candidate_pt]) != interested_status_list.end())
    {
      ...
      {  // choose the one with
        ...
        best_pos.second = pt_y;
                                                       ...
                                                       // are the same
      {
        float best_dist = std::sqrt(pow(best_pos.first, 2) + pow(best_pos.second, 2));
...
Point2D Robot::estimateNextStep(ScanData scan, Point2D end_pos, float max_dist, bool &call_help)
{
  ...
  // y value!
  {
...

    {
      for (int i = 1; i <= num_steps; i++)
      {
        ...
        if (heading_ > 10e-2 && heading_ - M_PI / 2.0 < -10e-2)
        {  // 1st quadrant
          y = float(y_ + i);
        ...
        else if (heading_ < -10e-2 && heading_ + M_PI / 2.0 < -10e-2)
        {  // 4th quadrant
          y = float(y_ - i);
          x = (y - y_) / ratio + float(x_);
          line_point_vec.insert({ floor(x), y });
        ...
        else if (std::abs(std::abs(heading_) - M_PI / 2.0) < 10e-2)  // heading == +- M_PI
  ...
  if (best_pos.first != farthest_pos.first || best_pos.second != farthest_pos.second)
  {
    call_help = true;
  }
