...
public:
  CommunicationInterface(ros::NodeHandle &nh) {
    ...
    pos_subscriber_ = nh.subscribe(
        "/odom", 1, &CommunicationInterface::robotPositionCallback, this);
    ...
    pub_map_quadrant_1_ =
        nh.advertise<nav_msgs::OccupancyGrid>("occu_map_quadrant_1", 100);
    pub_map_quadrant_2_ =
        nh.advertise<nav_msgs::OccupancyGrid>("occu_map_quadrant_2", 100);
    pub_map_quadrant_3_ =
        nh.advertise<nav_msgs::OccupancyGrid>("occu_map_quadrant_3", 100);
    pub_map_quadrant_4_ =
        nh.advertise<nav_msgs::OccupancyGrid>("occu_map_quadrant_4", 100);

    // debug output: processed scan_points
    scan_publisher_ =
        nh.advertise<sensor_msgs::PointCloud>("/processed_scan", 1);

    // message_filters::Subscriber<sensor_msgs::LaserScan> scan_input(nh_,
    // "/scan", 3);

    // message_filters::Subscriber<nav_msgs::Odometry> odom_input(nh_, "/odom",
    // 3);

    // message_filters::TimeSynchronizer<sensor_msgs::LaserScan,
    // nav_msgs::Odometry> sync(scan_input, odom_input, 3);
...
private:
  ...
  std::vector<std::pair<float, float>> curr_scan_;
...
public:
  void laserScanCallback(const sensor_msgs::LaserScan::ConstPtr &scan) {
    ...
    int counter = 0;
    int resolution =
        10; // pick only every ten points outside the interested area
    for (int i = 0; i < number_of_laser; i++) {
      ...
      tf::Quaternion q(robot_pose_.orientation.x, robot_pose_.orientation.y,
                       robot_pose_.orientation.z, robot_pose_.orientation.w);
      tf::Matrix3x3 m(q);
      double roll, pitch, yaw;
      m.getRPY(roll, pitch, yaw);
      double robot_heading = yaw;
      if (laser_angle > -priority_angle_range_rad &&
          laser_angle < priority_angle_range_rad) {
        ...
        {
          // check the +- 10 points
          if ((priority_angle_range_rad - abs(laser_angle)) / angle_increment <
              10) {
            continue;
          } else {
            bool valid = true;
            for (int j = 0; j < 10; j++) {
              if (!std::isnan(scan->ranges[i + j]) ||
                  !std::isnan(scan->ranges[i - j])) {
                valid = false;
                break;
              }
            }
    ...
    if (reached_pos_) {
      ...
      std::cerr << curr_robot_pos_.first << " " << curr_robot_pos_.second << " "
                << estimated_pos_.first << " " << estimated_pos_.second
                << std::endl;
    }
  ...
  void robotPositionCallback(const nav_msgs::Odometry msg) {
    ...
    // transform estimated_pos_ into absolute pos
    if (reached_pos_) {
      goal_.first = estimated_pos_.first * std::cos(yaw) -
                    estimated_pos_.second * std::sin(yaw) + pos_x;
      goal_.second = estimated_pos_.first * std::sin(yaw) +
                     estimated_pos_.second * std::cos(yaw) + pos_y;
    }
    // goal_.first = 10;
    // goal_.second = 10;
    ...
    if (fabs(curr_robot_pos_.first - goal_.first) > 1e-1 ||
        fabs(curr_robot_pos_.first - goal_.first) > 1e-1) {
      std::cerr << "not reached yet" << std::endl;
      std::cerr << "curr pos: " << curr_robot_pos_.first << " "
                << curr_robot_pos_.second << " "
                << "next pos: " << goal_.first << " " << goal_.second
                << std::endl;
      reached_pos_ = false;
